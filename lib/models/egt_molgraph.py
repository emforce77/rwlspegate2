...
class EGT_MOL(EGT):
    def __init__(self,
        ...
        if self.svd_encodings:
            self.svd_embed = nn.Linear(self.svd_encodings, self.edge_width) #*2  node_width

            self.pos_embed = nn.Linear(3*self.svd_encodings//4, self.edge_width)
            self.cls_embed = nn.Linear(self.svd_encodings//4, self.edge_width)
        
        self.dist_embed = nn.Embedding(self.upto_hop+2, self.edge_width)
        self.featm_embed = nn.Embedding(self.svd_encodings,
    ...
    def input_block(self, inputs):
        ...
        if self.svd_encodings:
            h = h #+ self.svd_embed(g.svd_encodings.float())

        
        
        e = self.dist_embed(dm)\
              + self.featm_embed(featm).sum(dim=3) 
                   # (b,i,j,f,e) -> (b,i,j,e)

        ss = g.svd_encodings.shape

        p = self.pos_embed(g.svd_encodings.view(ss[0], ss[1], -1, self.svd_encodings).float()[:,:,:, :24])
        c = self.cls_embed(g.svd_encodings.view(ss[0], ss[1], -1, self.svd_encodings).float()[:,:,:, 24:])
