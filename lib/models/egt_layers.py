...
class EGT_Layer(nn.Module):
    ...
    def __init__(self,
        ...
        self.mha_ln_p   = nn.LayerNorm(self.edge_width)
        self.mha_ln_c   = nn.LayerNorm(self.edge_width)

        self.lin_E      = nn.Linear(self.edge_width, self.num_heads)

        self.lin_concat = nn.Linear(self.edge_width * 3, self.edge_width) ###
        ...
        if self.edge_update:
            ...
            self.lin_O_p    = nn.Linear(self.num_heads, self.edge_width) ##
            self.lin_O_c = nn.Linear(self.num_heads, self.edge_width)  ##


            if self.edge_mha_dropout > 0:
                self.mha_drp_e  = nn.Dropout(self.edge_mha_dropout)

                self.mha_drp_p = nn.Dropout(self.edge_mha_dropout) ##
                self.mha_drp_c = nn.Dropout(self.edge_mha_dropout) ##
            ...
            pos_inner_dim = round(self.edge_width * self.edge_ffn_multiplier)
            self.ffn_ln_p = nn.LayerNorm(self.edge_width)
            self.lin_W_p_1 = nn.Linear(self.edge_width, pos_inner_dim)
            self.lin_W_p_2 = nn.Linear(pos_inner_dim, self.edge_width)

            cls_inner_dim = round(self.edge_width * self.edge_ffn_multiplier)
            self.ffn_ln_c = nn.LayerNorm(self.edge_width)
            self.lin_W_c_1 = nn.Linear(self.edge_width, cls_inner_dim)
            self.lin_W_c_2 = nn.Linear(cls_inner_dim, self.edge_width)
            ...
            if self.edge_ffn_dropout > 0:
                ...
                self.ffn_drp_p = nn.Dropout(self.edge_ffn_dropout)
                self.ffn_drp_c = nn.Dropout(self.edge_ffn_dropout)
    
    def forward(self, g):
        h, e, p, c = g.h, g.e, g.p, g.c #############
        mask = g.mask
        ...
        p_ln = self.mha_ln_p(p)  ####
        c_ln = self.mha_ln_c(c)  ####
        
        QKV = self.lin_QKV(h_ln)

        e_ln = torch.cat((e_ln,p_ln,c_ln) , dim=-1) ##########
        e_ln = self.lin_concat(e_ln)    
        ...
        if self.edge_update:
            ...
            p = self.lin_O_p(H_hat) ###lin_O_p
            if self.edge_mha_dropout > 0:
                p = self.mha_drp_p(p)###mha_drp_p
            p.add_(p_r1)

            p_r2 = p
            p_ln = self.ffn_ln_p(p) ###ffn_ln_p
            p = self.lin_W_p_2(self.ffn_fn(self.lin_W_p_1(p_ln))) ###
            if self.edge_ffn_dropout > 0:
                p = self.ffn_drp_p(p)
            p.add_(p_r2)
            ...
            c = self.lin_O_c(H_hat)
            if self.edge_mha_dropout > 0:
                c = self.mha_drp_c(c)
            c.add_(c_r1)

            c_r2 = c
            c_ln = self.ffn_ln_c(c)
            c = self.lin_W_c_2(self.ffn_fn(self.lin_W_c_1(c_ln)))
            if self.edge_ffn_dropout > 0:
                c = self.ffn_drp_c(c)
            c.add_(c_r2)
...
class VirtualNodes(nn.Module):
    def __init__(self, node_width, edge_width, num_virtual_nodes = 1):
        ...
        #########################
        self.vn_pos_embeddings = nn.Parameter(torch.empty(num_virtual_nodes,
                                                           self.edge_width))
        self.vn_cls_embeddings = nn.Parameter(torch.empty(num_virtual_nodes,
                                                           self.edge_width))
        ###########################
        ...
        ##################################
        nn.init.normal_(self.vn_pos_embeddings)
        nn.init.normal_(self.vn_cls_embeddings)
        #####################################
    ...
    def forward(self, g):
        ...
        #################################################################
        p_shape = p.shape
        pos_emb_row = self.vn_pos_embeddings.unsqueeze(1)
        pos_emb_col = self.vn_pos_embeddings.unsqueeze(0)
        pos_emb_box = 0.5 * (pos_emb_row + pos_emb_col)

        pos_emb_row = pos_emb_row.unsqueeze(0).expand(p_shape[0], -1, p_shape[2], -1)
        pos_emb_col = pos_emb_col.unsqueeze(0).expand(p_shape[0], p_shape[1], -1, -1)
        pos_emb_box = pos_emb_box.unsqueeze(0).expand(p_shape[0], -1, -1, -1)

        p = torch.cat([pos_emb_row, p], dim=1)
        p_col_box = torch.cat([pos_emb_box, pos_emb_col], dim=1)
        p = torch.cat([p_col_box, p], dim=2)

        ###

        c_shape = c.shape
        cls_emb_row = self.vn_cls_embeddings.unsqueeze(1)
        cls_emb_col = self.vn_cls_embeddings.unsqueeze(0)
        cls_emb_box = 0.5 * (cls_emb_row + cls_emb_col)

        cls_emb_row = cls_emb_row.unsqueeze(0).expand(c_shape[0], -1, c_shape[2], -1)
        cls_emb_col = cls_emb_col.unsqueeze(0).expand(c_shape[0], c_shape[1], -1, -1)
        cls_emb_box = cls_emb_box.unsqueeze(0).expand(c_shape[0], -1, -1, -1)

        c = torch.cat([cls_emb_row, c], dim=1)
        c_col_box = torch.cat([cls_emb_box, cls_emb_col], dim=1)
        c = torch.cat([c_col_box, c], dim=2)
